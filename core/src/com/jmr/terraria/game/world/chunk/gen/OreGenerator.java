...
public class OreGenerator implements WorldGenerator {
	...
	public void generate(SimplexNoise noise, com.jmr.terraria.game.world.chunk.Chunk chunk) {
		for (int y = 0; y < com.jmr.terraria.game.world.chunk.Chunk.CHUNK_SIZE; y++) {
	...
	public void generate(SimplexNoise noise, com.jmr.terraria.game.world.chunk.Chunk chunk, int x, int y) {
		...
		int totalY = y + (com.jmr.terraria.game.world.chunk.Chunk.CHUNK_SIZE * chunk.getStartY());

		float caveFreq = 1.0f / (com.jmr.terraria.game.world.chunk.Chunk.CHUNK_SIZE / 3.85f);
		float caveVal = Math.abs(noise.generate(totalX * caveFreq, totalY * caveFreq, 4, .5f, 1f));

		if (caveVal <= .35f) {

			float oreFreq = 1.0f / (com.jmr.terraria.game.world.chunk.Chunk.CHUNK_SIZE / 3.5f);
